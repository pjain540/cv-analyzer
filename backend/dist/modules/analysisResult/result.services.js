...
export const performAnalysis = async (resumeIds, jobDescriptionId, userId) => {
    ...
    const analysisResults = await Promise.all(resumes.map(async (resume) => {
...
export const getAllAnalysis = async (page, limit, pagination = true, userId) => {
    const results = await getPaginatedData(AnalysisResult, { user: userId }, page, limit, { createdAt: -1 }, pagination, "jobDescription");
    return results;
};
//# sourceMappingURL=result.services.js.map